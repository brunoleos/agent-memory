...
O Manifest é verificável, não especulativo. Cada arquivo é uma **microspec**: um contrato observável por capacidade, que nasce em duas direções (ADR-0052) — **bottom-up**, destilado no debrief do trabalho feito, ou **top-down**, prospectado como feature `proposed` com critérios EARS completos antes de qualquer código. A invariante que separa as duas é estrutural: critério em feature `in_progress`/`shipped` é afirmação viva sobre o presente (refutável pela amostragem adversarial); prospecção vive em feature `proposed` própria (com `depends_on` quando estende capacidade existente), isenta de drift de contracts (ADR-0044) mas com relógio anti-apodrecimento — `proposed` sem referência em UNRELEASED/`ideas.md` por 90+ dias gera nudge `info`. A microspec nunca é driver de workflow: o feat-memory não gera plano, não decompõe tasks, não implementa a partir da spec — plan/task são competência do harness. A estrutura é uma pasta com um arquivo por feature mais um índice gerado automaticamente, simétrica à pasta `.feat-memory/decisions/`.
...
O campo `acceptance` é uma lista de critérios estruturados na notação EARS. Cada critério tem um `id` estável e um `pattern` que determina os campos obrigatórios. A notação distingue cinco situações qualitativamente diferentes em que um requisito pode se aplicar — invariante sempre ativa (`ubiquitous`), estímulo-resposta (`event`), condicional a estado (`state`), comportamento opcional (`optional`), condição indesejada (`unwanted`) — mais o escape `complex` para combinações, a usar com parcimônia. Explicitar o padrão elimina a ambiguidade sobre *quando* o requisito está em vigor.

Os campos obrigatórios de cada padrão e as formulações canônicas vivem na referência gerada — [docs/SCHEMA-REFERENCE.md](docs/SCHEMA-REFERENCE.md), ou `feat-memory schema` — que é sincronizada com o validador por teste (fonte única mecânica; re-listar os campos aqui seria a cópia que drifta). A validação no `feat-memory audit` exige `pattern` declarado e os campos daquele padrão presentes e não-vazios; critérios mal-formados bloqueiam o build.

Um teste pode declarar cobrir um critério específico citando o token `F-NNNN-AN` (convenção opt-in de binding, F-0049): o audit mede a cobertura por critério e o `sample` aponta o teste declarado no prompt de refutação — presença referencial, nunca execução.
...
A metodologia inclui quatro skills em `skills/` na raiz do workspace (deployadas pelo `feat-memory deploy` a partir do package data). Elas são opcionais — o protocolo está documentado neste arquivo — mas tornam a aplicação consistente. **O procedimento de cada fluxo vive no respectivo `SKILL.md`, que é a fonte única e autoritativa** (o frontmatter traz os triggers de ativação); re-narrar os passos aqui seria memória paralela, e memória paralela drifta. Esta seção define apenas o **contrato** de cada skill — quando existe e o que garante:

- **`memory-deploy`** — adoção inicial (greenfield ou legacy). Garante: estrutura mecânica via `feat-memory deploy`; frontmatter *proposto para aprovação*, nunca gravado sem aval humano (ADR-0032); em legacy, gênese retroativa revisada fase a fase (ADR-0030/0031) — cristalização silenciosa de interpretação errada é o erro que ela existe para impedir.
- **`memory-bootstrap`** — início de sessão. Garante: retomada dentro do orçamento — carrega o UNRELEASED e expande só o que ele referencia (ADR-0043); fallback no `ideas.md` quando nada está em voo; delegação a `memory-pull-brief` pós-merge.
- **`memory-debrief`** — fim de unidade de trabalho; a mais usada. Garante: Manifest e UNRELEASED movidos junto com o código; ADR proposto quando a sessão decidiu algo; propagação de supersede reconciliada; retrospectiva com triagem de ideias (ADR-0047/0048); e o teste do agente frio como pergunta final (ADR-0050).
- **`memory-pull-brief`** — pós-`git pull`. Garante: reconciliação do UNRELEASED local com o que veio do remote; read-only sobre `manifest/` e `decisions/` (que já vieram corretos do pull).

A separação em quatro reflete quatro momentos qualitativamente diferentes — adoção, início, fim, sincronização — cada um com seus próprios riscos de execução errada; skills monolíticas tendem a ser ignoradas. E a *skill* (não o comando) é o ponto de entrada da adoção porque o comando sozinho instala estrutura sem personalizar nada; o comando direto fica para automação e CI.

## Deploy

O `feat-memory deploy` é idempotente em todas as superfícies que instala — rodar duas vezes é seguro, e re-rodar após um upgrade da CLI é o caminho normal de atualização do consumidor.

O `AGENTS.md` carrega um bloco delimitado por sentinelas markdown (`<!-- >>> feat-memory >>> -->` … `<!-- <<< feat-memory <<< -->`), refrescado a cada deploy; **todo o resto do arquivo** — frontmatter aprovado, seções do mantenedor — é preservado byte a byte. `CLAUDE.md` (redirect `@AGENTS.md`) é copiado se ausente e deixado quieto se existe. O `changelog/UNRELEASED.md` nunca é sobrescrito quando existe (regra do postmortem ADR-0051); o header do `ideas.md` é refrescado preservando as entradas do usuário. As skills são reescritas a cada deploy quando divergem do template — são conteúdo de metodologia, não de usuário; quem quiser uma variante customizada copia a skill para outro nome (o deploy só escreve nos nomes canônicos, então a cópia do usuário nunca é tocada). `.gitattributes` e `.gitignore` usam blocos com sentinelas; o pre-commit hook é (re)instalado; o `.meta.yaml` registra versão, timestamp e perfil.

Flags: `--profile core|full` (seção seguinte); `--no-merge` pula a refresh do bloco em `AGENTS.md`/`CLAUDE.md` existentes (útil em CI); `--force` reescreve `AGENTS.md`/`CLAUDE.md` inteiros do template, descartando conteúdo do usuário fora do bloco — escolha consciente do operador, nunca default; `--no-hooks` e `--dry-run` fazem o que dizem.
